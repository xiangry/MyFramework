...
public class PackageTool : EditorWindow
{
    ...
    static private Manifest manifest = null;
    ...
    void DrawAssetBundlesConfigGUI()
    {
        ...
        if (GUILayout.Button("更新Prefab标签", GUILayout.Width(200)))
        {
...
//        }
    ...
    void DrawBuildAndroidPlayerGUI()
    {
        ...
        else
        {
            ...
            if (GUILayout.Button("游戏打包", GUILayout.Width(200)))
            {
                EditorApplication.delayCall += BuildAndroidPlayerForCurrentChannel;
            }
            if (GUILayout.Button("Open Output Folder", GUILayout.Width(200)))
    ...
    public static void CheckAllDependenciesRes()
    {
        // 写文件
        StreamWriter sw;
        FileInfo fi = new FileInfo("Assets/StreamingAssets/dependencies_res.txt");
        if (!fi.Exists)
        {
            Debug.Log("写入 不存在");
            sw = fi.CreateText();
        ...
        WriteAllDependenciesRes(sw, false);
        WriteAllDependenciesRes(sw, true);
