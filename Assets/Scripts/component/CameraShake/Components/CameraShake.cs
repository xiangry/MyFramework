...
public class CameraShake : MonoBehaviour
{
    ...
    public void DoShake(int numberOfShakes, Vector3 shakeAmount, Vector3 rotationAmount, float distance, float speed, float decay, bool multiplyByTimeScale)
    {
        ...
        foreach (Camera cam in cameras)
        {
            StartCoroutine(DoShake_Internal(cam, seed, numberOfShakes, shakeAmount, rotationAmount, distance, speed, decay, this.guiShakeModifier, multiplyByTimeScale, null));
        }
    }
...
//            if (cam.transform.parent && cam.transform.parent.GetComponent<UIRoot>() != null)
            if (cam.transform.parent && cam.transform.parent.name == "UIRoot")
            {
                // shake camera which is child of UIRoot for NGUI
